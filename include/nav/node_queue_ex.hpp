...
public:
    NodeQueueEx(
        size_t capacity
    ) : m_capacity(capacity), m_size(0), m_nodes(new NodeEx * [capacity]) {
    ...
    insert_node(
        ...
        while (current_index != 0 && m_nodes[get_parent_index(current_index)]->m_f > m_nodes[current_index]->m_f) {
...
private:
    ...
    heapify(
        ...
        if (l < m_size && m_nodes[l]->m_f < m_nodes[smallest]->m_f) {
            smallest = l;
        }
        if (r < m_size && m_nodes[r]->m_f < m_nodes[smallest]->m_f) {
            smallest = r;
        }
